...
<html>
	<head>
		...
		function checkusername(username){
			if(username == ""){
				...
				xmlHttp.open("post", "/MyProject_Demo_one/buyer_servlet/buyer_checkusername?username=" + username);	//设置请求
		...
		function checknickname(nickname){
			if(nickname == ""){
				document.getElementById("text2").innerHTML = "昵称不能为空!";
				flag2 = false;	//无法提交表单
			}else{
				createXMLHttp();	//创建xmlHttp
				xmlHttp.open("post", "/MyProject_Demo_one/buyer_servlet/buyer_checknickname?nickname=" + nickname);	//设置请求
				xmlHttp.onreadystatechange = checknicknameCallback;	//设置回调函数
				xmlHttp.send(null);	//发送请求
		...
		function checknicknameCallback(){
			if(xmlHttp.readyState == 4){	//数据返回完毕
				if(xmlHttp.status == 200){	//HTTP操作正常
					var text = xmlHttp.responseText;	//接受返回内容
					if(text == "true"){
						document.getElementById("text2").innerHTML = "恭喜!该昵称可以注册";
						flag2 = true;	//可以提交表单
					}else{
						document.getElementById("text2").innerHTML = "昵称重复!请重新输入";
						flag2 = false;	//无法提交表单
					}
				}
			}
	...
	<body>
		<div align="center" class="log0">&nbsp; 
			<span class="login_span">买家注册</span>
		</div>
		<br />
		<br />
		<div class="log1">
			<form action="/MyProject_Demo_one/buyer_servlet/buyer_register_servlet" method="post" onsubmit="return checkForm()">
				<div class="form_left">
					...
					<img class="register" src="../img/register.jpg">
	 				...
	 				<div>
	 					<span>昵称：</span> <input type="text" name="nickname" placeholder="nickname" onblur="checknickname(this.value)" />
	 				</div>
	 				...
	 				<div>
	 					<span>验证码:</span> <input type="text" name="validate" id="validate" placeholder="validate"/>
	 				</div>
		...
		<div class="log2">
			<a href="buyerLogin.jsp" class="a_back">返回登陆页面</a>
		</div>
		<div class="log3">
